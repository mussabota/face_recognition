...
#                print('Detected_FaceNum: %d' % nrof_faces)
                ...
                if nrof_faces == 0:
                    ...
                    for i in range(nrof_faces):
                        ...
                        cv2.rectangle(frame, (bb[i][0], bb[i][1]), (bb[i][2], bb[i][3]), colors.green, 2)  # boxing face
                        ...
                        # print(curTime)
                        name, surname, user_id = str(HumanNames[best_class_indices[0]]).split('_')
                        ...
                        if best_class_probabilities > 0.7:
                            ...
                            cv2.putText(frame, "Accuracy is {0}%".format(int(best_class_probabilities * 100)), (0, 20),
                            ...
                            time.sleep(3)
                            print(result_names, 'last entered at ', fixed_time)
                            cv2.putText(frame, result_names + " last entered at {0}".format(fixed_time),
                                        # (0, 220), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 1) """
                            ...
                            if cv2.waitKey(1) & 0xFF == ord('r'):
                                ...
                                cv2.putText(frame, result_names + " last entered at {0}".format(fixed_time),
                                            ...
                                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 1)
                                #for H_I in HumanNames:
                                 #   li = tuple(H_I.split('_'))

                                    # print(li)
                                me = Users(first_name=name, last_name=surname, user_id=user_id, last_entered=fixed_time)
                                db_session.add(me)
